...
class single extends responsetype {
    ...
    static public function answers_from_webform($responsedata, $question) {
        ...
        if (isset($responsedata->{'q' . $question->id}) && isset($question->choices[$responsedata->{'q' . $question->id}])) {
            ...
            if ($question->choices[$responsedata->{'q' . $question->id}]->is_other_choice()) {
                $cname = 'q' . $question->id .
                    \mod_feedbackbox\question\choice\choice::id_other_choice_name($responsedata->{'q' . $question->id});
    ...
    static public function answers_from_appdata($responsedata, $question) {
        ...
        if (isset($responsedata->{$qname}[0]) && !empty($responsedata->{$qname}[0])) {
            ...
            if ($question->choices[$record->choiceid]->is_other_choice()) {
                $cname = \mod_feedbackbox\question\choice\choice::id_other_choice_name($record->choiceid);
                $record->value =
    ...
    /**
     * @param \mod_feedbackbox\responsetype\response\response|\stdClass $responsedata
     * @return bool|int
     * @throws \coding_exception
    ...
    public function insert_response($responsedata) {
        ...
        if (!$responsedata instanceof \mod_feedbackbox\responsetype\response\response) {
            $response = \mod_feedbackbox\responsetype\response\response::response_from_webform($responsedata,
                [$this->question]);
        } else {
            ...
            foreach ($response->answers[$this->question->id] as $answer) {
                if (isset($this->question->choices[$answer->choiceid])) {
                    if ($this->question->choices[$answer->choiceid]->is_other_choice()) {
                        ...
                        $DB->insert_record('feedbackbox_response_other', $record);
                    }
    ...
    public function get_results($rids = false, $anonymous = false) {
        ...
        $sql = 'SELECT rt.id, rt.response, qc.content ' .
            'FROM {feedbackbox_response_other} rt, ' .
            '{feedbackbox_quest_choice} qc ' .
    ...
    public function get_feedback_scores(array $rids) {
        ...
        $sql = 'SELECT response_id as rid, c.value AS score ' .
            ...
            'INNER JOIN {feedbackbox_quest_choice} c ON r.choice_id = c.id ' .
    ...
    public function results_template($pdf = false) {
        if ($pdf) {
            return 'mod_feedbackbox/resultspdf_choice';
        } else {
            return 'mod_feedbackbox/results_choice';
        }
    }
        ...
        if ($rows) {
            ...
            foreach ($rows as $idx => $row) {
                if (strpos($idx, 'other') === 0) {
                    ...
                    $content = \mod_feedbackbox\question\choice\choice::content_other_choice_display($ccontent);
    ...
    static public function response_select($rid) {
        ...
        foreach ($records as $row) {
            ...
            if (\mod_feedbackbox\question\choice\choice::content_is_other_choice($row->ccontent)) {
                $newrow['responses'][\mod_feedbackbox\question\choice\choice::id_other_choice_name($row->cid)] = $row->response;
            }
    ...
    static public function response_answers_by_question($rid) {
        ...
        $sql = 'SELECT r.id as id, r.response_id as responseid, r.question_id as questionid, r.choice_id as choiceid, ' .
            ...
            'LEFT JOIN {feedbackbox_response_other} o ON r.response_id = o.response_id AND r.question_id = o.question_id AND ' .
    ...
    public function get_bulk_sql($feedbackboxids,
      ...
      LEFT JOIN {feedbackbox_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrs.choice_id
